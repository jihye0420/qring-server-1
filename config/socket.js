...
app.io.on("connection", (socket) => {   // connection 이벤트 성공 => connection에 대한 정보를 담고 있는 변수 : socket
                                        // socket을 이용해서 event listener를 만듦
     ...
     * 유저가 나갔을 때
     */
    socket.on("leaveRoom", (roomName) => {  // leaveRoom이라는 이벤트 성공 => leaveRoom에 대한 정보를 담고 있는 변수 : roomName
        socket.leave(roomName, () => {
            console.log("leave");
            try {
                const itemToFind = socket.adapter.rooms[roomName].userList.find( // room에서 나간 사람 id 찾기
                    function (item) {
                        return item.userID === socket.id;
                    }
                );
                let idx = socket.adapter.rooms[roomName].userList.indexOf(itemToFind); // 나간 사람 idx
                if (idx > -1) {
                    if (socket.adapter.rooms[roomName].userList[idx].king == true) {    // 나간 사람이 방장일 때
                        console.log("king emit");
                        if (socket.adapter.rooms[roomName].userList[idx + 1]) {
                            socket.adapter.rooms[roomName].userList[idx + 1].king = true;   // 방장 위임하기
                        }
    ...
    socket.on("startGame", (roomName) => {
        if (!socket.adapter.rooms[roomName].userList) {} else {
            let userWrap = {};
            userWrap.userList = socket.adapter.rooms[roomName].userList;
            userWrap.question = randomItem(q);
    ...
    socket.on("vote", (roomName, isFront) => {
        ...
        if (voteNum <= 0) {
            ...
            app.io.in(roomName.toString()).emit("voteNum", {
                voteNum: voteNum
            });
        }
    });
