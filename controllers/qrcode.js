...
const qrcodeController = {
    ...
    qrCode.toFile(
      ...
      (err, string) => {
        ...
        s3.upload(param, async (err, data) => {
          ...
          await meetingModel.updateOne(filter, { $set: update });
  ...
  submitForm: async (req, res) => {
    ...
    const groupInfo = await groupModel.findById({_id : groupId }, { meetings : 1 });
    ...
    if (rounds == 1){ 
      ...
      if (!flag) {
        ...
        const filter = { _id: meetingId };
        const update = { user: { name, email, abroad, health, attendance, isAdded, now }};
        ...
        if (result){
          ...
          if (!isAdded){
            ...
            if (createdAt <= startTime || createdAt >= endTime){  
              const filter = { _id: meetingId, 'user.email': email };
              let update = { 
                'user.$.name' : name,
                'user.$.email' : email,
                'user.$.abroad' : abroad,
              ...
              await meetingModel.findOneAndUpdate(filter, {$set : update});
            } else {
